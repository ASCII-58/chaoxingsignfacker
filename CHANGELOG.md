...
- **手势签到功能** ✅
  - 支持 3×3 网格手势绘制
  - 手势验证机制
  - GestureSignComponent 组件
  - 手势签到图标 (gesture.svg)
- **签到码签到功能** ✅
  - 支持数字签到码输入
  - 可变位数签到码支持
  - PasswordSignComponent 组件
  - 签到码图标 (password.svg)
- **签退功能** ✅
  - GetSignInfo 方法获取签到详细信息
  - DoSignOut 方法执行签退
  - 签到成功后显示签退按钮
- **新增 API 接口**
  - URL_GET_SIGN_INFO - 获取签到详细信息（包含签退信息）
  - URL_CHECK_SIGN_CODE - 验证手势码/签到码是否正确
  - URL_SIGN_OUT - 签退接口
- **SignType 枚举扩展**
  - GESTURE = '3' - 手势签到
  - PASSWORD = '5' - 签到码签到
...
- **首页列表性能优化** ✅
  ...
  - 移除了未使用的状态变量，减少内存开销
- **签到页面架构优化** ✅
  - 统一所有签到类型在 SignerPage 中处理
  - 将手势签到和密码签到实现为可复用组件
  - 手势和密码签到使用组件内部按钮，隐藏页面底部按钮
  - 改进签到类型的判断和路由逻辑
